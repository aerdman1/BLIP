...
export const CURRENT_STATUS = [
  ...
  'Settings screen filters now apply to the title and active top-down world camera with a saved 0-100 intensity slider. Gameplay filters still cap at readability-safe strength; HUD/DOM overlays remain readable.',
...
export const VERTICAL_SLICE_SYSTEMS: SliceSystemItem[] = [
  ...
  { name: 'Phase Boost', status: 'PLAYABLE', note: 'Hold SHIFT/RB/touch Boost for a limited high-speed surge with boost-energy drain, regeneration, brief i-frames, restrained behind-the-player echoes and an explicit BOOST meter in the HUD.' },
  ...
  { name: 'Motel stealth onboarding', status: 'IN DEVELOPMENT', note: 'Scanner beams, alert state, Boost prompt and combat fallback are live. Alerts add pressure without direct beam damage, but Motel still needs reliable legitimate arrivals before broader testing.' },
  ...
  { name: 'Field-event reward pockets', status: 'PLAYABLE', note: 'All five route regions have visible purpose-labeled events that grant shards, health, overdrive, weapon prototypes or specific Scout boons, with optional ambush spawns and AI-visible perception markers. Markers now read as CACHE, RECOVERY, SCOUT TECH, WEAPON or OVERDRIVE instead of generic action plates.' },
...
export const BUILD_TODO: TodoItem[] = [
  ...
  { label: 'Environment-depth pass: shared HD terrain now adds biome-specific ground wear, medium/tall silhouettes, foreground framing and local named-area dressing without changing collision/routes', done: true },
...
export const ART_DIRECTION: string[] = [
  ...
  'The playable world renders through the top-down Phaser scene with y-sorted actors, readable cover, environmental obstacles, signal-colored affordances and layered low/medium/tall environment dressing.',
