...
export const CURRENT_STATUS = [
  ...
  'Focused environment-depth polish is live in the shared HD terrain path: biome-specific ground wear, cracks, grates, row marks, street markings, medium/tall edge silhouettes, foreground framing, roof/parapet/service-equipment detail, named-area dressing and data-driven elevation zones with subtle follow-camera offset/zoom now help signs like Main Street, Pool Courtyard, Orchard Rows, Raised Ridge and Classifier Core read as real places instead of labels on identical rooms. The ambiguous Motel/Town car landmark is no longer auto-placed until a clearer vehicle asset exists. This is additive dressing only; routes, collisions and encounters were preserved.',
  'Major weapon/reward/trophy cards now open as centered blocking read-and-continue modals. Gameplay pauses behind them and queued major rewards reveal one at a time, preventing reward text from overlapping route signs, combat HUD or pickup labels.',
...
export const MECHANICS: MechanicDef[] = [
  ...
  { name: 'Elevation Zones', description: 'Raised ridges, rooflines, creek lows and rift bowls are data-driven 2.5D zones with visible ground-layer cues and subtle follow-camera offset/zoom so maps feel less flat without changing collision architecture.' },
...
export const VERTICAL_SLICE_SYSTEMS: SliceSystemItem[] = [
  ...
  { name: 'Elevation and visual depth', status: 'IN DEVELOPMENT', note: 'All five route regions have data-driven rise/drop/roofline/creek/rift zones with ground-layer cues and subtle camera offset/zoom. This creates hills/lows/ridges without full 3D physics or route rewrites.' },
...
export const BUILD_TODO: TodoItem[] = [
  ...
  { label: 'Environment-depth pass: shared HD terrain now adds biome-specific ground wear, medium/tall silhouettes, foreground framing, local named-area dressing and data-driven elevation zones with subtle follow-camera offset/zoom without changing collision/routes', done: true },
