...
export const CURRENT_STATUS = [
  ...
  'Focused AI campaigns are wired for region and route slices. Latest public AI JSON is `first-three-route-focus-v3`: 6 Miller→Motel→Town attempts, 3/6 completions, 0 deaths and 1 soft-lock-risk stall, with `circuit-z2` route-forwarding still the main friction area. Reports now include decision traces and stall samples so failures are inspectable. Orchard-focused `orchard-ai-gravity-priority-v1` remains 6/6. AI numbers are design evidence, not a hard deploy gate.',
...
export const MASTER_BACKLOG_CRITICAL_PATH: BacklogPathItem[] = [
  ...
  { priority: 'P1', item: 'AI safety and route guidance after layouts', status: 'PARTIAL', note: 'AI personas use visible signal markers, labeled objective hints, scanners, enemies, visible enemy roles and pickups from perception snapshots. Campaign runner uses isolated strict preview ports and reports decision traces/stall samples. Orchard-focused AI remains 6/6; first-three route evidence improved but remains partial.' },
  { priority: 'P0', item: 'First-route clarity after layout pass', status: 'PARTIAL', note: '`first-three-route-focus-v3` reached 3/6 Miller→Motel→Town completions with 0 deaths and 1 soft-lock-risk stall after route-focused AI behavior and better soft-lock classification. Failed lower-road breadcrumb relocation collapsed completion to 0/6 and was reverted. Needs stronger visible route/exit guidance and Motel/Town objective clarity.' },
...
export const VERTICAL_SLICE_SYSTEMS: SliceSystemItem[] = [
  ...
  { name: 'AI Player Lab', status: 'TESTED', note: 'Personas, campaign runner, evidence JSON and E2E coverage are live. Latest public run `first-three-route-focus-v3` reached 3/6 Miller→Motel→Town completions with 0 deaths and 1 soft-lock-risk stall. Reports include decision traces and stall samples. Orchard-focused `orchard-ai-gravity-priority-v1` remains 6/6.' },
];
...
export const BUILD_TODO: TodoItem[] = [
  ...
  { label: 'First-three route-focus evidence: `first-three-route-focus-v3` reached 3/6 Miller→Motel→Town completions with 0 deaths and 1 soft-lock-risk stall', done: true },
