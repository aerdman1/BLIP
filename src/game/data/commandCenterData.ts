...
export const CURRENT_STATUS = [
  ...
  'Focused AI campaigns are wired for region and route slices. Latest public AI JSON is `first-three-route-focus-v6`: 6 Miller→Motel→Town attempts, 6/6 completions, 0 deaths and 0 soft-lock-risk stalls after visible-objective AI focus, a fairer expanded-route time budget, earlier Motel recovery and first-route sign cleanup. Reports include decision traces and stall samples so failures are inspectable. Orchard-focused `orchard-ai-gravity-priority-v1` remains 6/6. AI numbers are design evidence, not a hard deploy gate.',
  'Schematic-first expanded layouts are implemented across all five route regions. The latest gameplay-design/content pass gives every named area a purpose: main route, optional branch, secret pocket, shortcut/return loop, worthwhile reward pocket, intentional enemy spaces, field-event rewards, visible scan/power markers, subtle discovered-area lighting, clear objective completion and satisfying exit approach. Motel scanner labels now explicitly read as scanner hazards and disappear from player/AI perception once the route opens. Pickup/event labels reserve space around route signs, and qa:maps now catches obvious route-sign label crowding. Manual live feel review is still needed before calling the maps final.',
...
export const MASTER_BACKLOG_CRITICAL_PATH: BacklogPathItem[] = [
  ...
  { priority: 'P1', item: 'AI safety and route guidance after layouts', status: 'PARTIAL', note: 'AI personas use visible signal markers, labeled objective hints, scanners, enemies, visible enemy roles and pickups from perception snapshots. Campaign runner uses isolated strict preview ports and reports decision traces/stall samples. Orchard-focused AI remains 6/6; first-three route evidence is now 6/6 on the focused same-seed sample. Broader regression and full-route evidence remain pending.' },
  { priority: 'P0', item: 'First-route clarity after layout pass', status: 'PARTIAL', note: '`first-three-route-focus-v6` reached 6/6 Miller→Motel→Town completions with 0 deaths and 0 soft-lock-risk stalls after visible-objective AI focus, fairer expanded-route timing, earlier Motel recovery and route sign cleanup. Failed lower-road and Check-Out breadcrumb experiments were reverted after worse samples. Needs broader regression plus subjective live feel review.' },
  { priority: 'P0', item: 'Unified notification/reward UI', status: 'PARTIAL', note: 'Objective card is the primary instruction surface, routine toasts dedupe into a smaller activity feed, combat toasts are capped at two with shorter dwell, center HUD banners queue one at a time, and reward banners queue one at a time. A single shared notification manager/showcase is still unfinished.' },
  { priority: 'P0', item: 'Major reward presentation', status: 'PARTIAL', note: 'Named region rewards use major reward banners and important weapon drops now show name, combat role and a queued weapon reward card. Random health drops no longer carry stale weapon ids, fixing pickup telemetry/UI mismatches. Full compare/equip/store/salvage flow is still deferred.' },
...
export const VERTICAL_SLICE_SYSTEMS: SliceSystemItem[] = [
  ...
  { name: 'Major loot presentation', status: 'NEEDS POLISH', note: 'Region rewards use queued major banners and important weapon pickups show resolved names/roles plus a queued acquire card from the actual equipped weapon id. Health pickups no longer carry stale weapon ids. Full compare/equip/store/salvage flow is not built yet.' },
  ...
  { name: 'First-route clarity', status: 'NEEDS POLISH', note: 'Phase-specific holographic route signposts are live and simplified. Broad road auto-warp zones were removed; transitions require a short hold inside the actual charged breach. Focused first-three AI now reaches Town 6/6 with 0 deaths/soft locks; continue broader regression and subjective route feel review.' },
  ...
  { name: 'AI Player Lab', status: 'TESTED', note: 'Personas, campaign runner, evidence JSON and E2E coverage are live. Latest public run `first-three-route-focus-v6` reached 6/6 Miller→Motel→Town completions with 0 deaths and 0 soft-lock-risk stalls. Reports include decision traces and stall samples. Orchard-focused `orchard-ai-gravity-priority-v1` remains 6/6.' },
];
...
export const BUILD_TODO: TodoItem[] = [
  ...
  { label: 'First-three route-focus evidence: `first-three-route-focus-v6` reached 6/6 Miller→Motel→Town completions with 0 deaths and 0 soft-lock-risk stalls', done: true },
