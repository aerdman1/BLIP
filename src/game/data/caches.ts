 ...
 * Each cache is a weighted drop table over rarities (+ optional category bias
 * and a guaranteed rarity floor). The roll logic in systems/RewardSystem.ts
 * reads these; the opening screen in ui/RewardUI.ts reads them for color/feel.
 */
...
export interface CacheDef {
  ...
  color: string;
  glow: string;
  /** how many rewards this cache drops */
  drops: number;
