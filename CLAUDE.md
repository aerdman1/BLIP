...
- Testing vocabulary: `npm run typecheck`, `npm run build`, and Playwright suites are hard pass/fail deploy gates. `npm run ai:campaign` is a heuristic playtest tool; its completion percentages are evidence for design priorities, not proof that the game is “passing.” Latest public AI JSON is `first-three-commit-scanner-v1`: 6 Miller→Motel→Town attempts, 1/6 completions, 0 deaths, 2 soft-lock-risk stalls, mostly in Motel route-forwarding. Latest Miller→Motel focused sample `route-open-commit-scanner-v1` reached 2/6 with 0 deaths/soft locks. Orchard-focused evidence remains `orchard-ai-gravity-priority-v1`: 6/6 completions with 0 soft locks/deaths. Extended AI campaigns remain pending until first-route completion is stronger.
