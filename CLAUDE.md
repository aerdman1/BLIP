...
- Current combat foundation: Pulse Carbine, Arc Blade and Recall Disc with fast switching. Phase Shift is live as the SHIFT short-range blink with restrained behind-player echoes; keep "dash" wording only for internal ids. Region goals/rewards are named in HUD/telemetry. Orchard has the first playable Gravity Well/raised-ridge traversal beat. Signal Tubes, Phase Doors, Signal Rails, deeper stealth, underground areas and larger puzzle chains remain scoped future work, not broad expansion.
...
- Current first-route focus: Miller Surface layout should be preserved. Miller is now gentler onboarding with no early elite beam, but focused AI still stalls after `breach=true` before reliably entering Motel. Improve visible east-road route-following first; then judge Motel-to-Town clarity and stealth identity.
- Testing vocabulary: `npm run typecheck`, `npm run build`, and `npm run test:e2e` are hard pass/fail deploy gates. `npm run ai:campaign` is a heuristic playtest tool; its completion percentages are evidence for design priorities, not proof that the game is “passing.” Extended AI campaigns are paused until an overnight run.
