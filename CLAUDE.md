...
- Testing vocabulary: `npm run typecheck`, `npm run build`, and Playwright suites are hard pass/fail deploy gates. `npm run ai:campaign` is a heuristic playtest tool; its completion percentages are evidence for design priorities, not proof that the game is “passing.” Latest public AI JSON is `route-exit-sign-fix-v1`: 6 Miller→Motel runs, 2/6 completions, 0 soft locks, 0 deaths, improving over the 0/6 `route-exit-duration-check-v1` baseline after the Miller exit breadcrumb was changed from a diagonal Motel Bend path to East Road → Breach Road → Motel Breach. Orchard-focused evidence remains `orchard-ai-gravity-priority-v1`: 6/6 completions with 0 soft locks/deaths. Extended AI campaigns remain pending until first-route completion is stronger.
