...
- Testing vocabulary: `npm run typecheck`, `npm run build`, and Playwright suites are hard pass/fail deploy gates. Run separate Playwright suites sequentially because the shared `webServer` uses one preview port; the standalone `npm run ai:campaign` runner is safe to isolate because it uses process-specific strict preview ports. `npm run ai:campaign` is a heuristic playtest tool; its completion percentages are evidence for design priorities, not proof that the game is “passing.” Latest public AI JSON is `first-three-route-focus-v3`: 6 Miller→Motel→Town attempts, 3/6 completions, 0 deaths, 1 soft-lock-risk stall, mostly in Motel route-forwarding. Reports now include objective target labels, decision traces and stall samples. Orchard-focused evidence remains `orchard-ai-gravity-priority-v1`: 6/6 completions with 0 soft locks/deaths. Extended AI campaigns remain pending until first-route completion is stronger.
