...
- Current layout focus: schematic-first expanded layouts plus the first named-area gameplay/content pass are implemented across Miller, Motel, Town, Orchard and Signal Storm. All five are larger without changing actor/camera/prop scale, and route/scanner/gravity helper coordinates are data-driven. The maps now track main routes, optional branches, secret pockets, shortcuts/loops, reward pockets, encounter spaces, field-event scan/power rewards, early weapon prototypes, subtle discovered-area dimming and exit approaches. HD blocked edges use biome-specific decorative dressing so boundaries read as woods, motel clutter, town obstacles, orchard rows or signal wreckage instead of identical square walls. Next, do manual feel review for travel time, empty space, cover, route readability and reward motivation before AI route-guidance/persona tuning or longer AI comparisons; during layout work use `npm run qa:maps`, typecheck/build/E2E, smoke tests and deterministic technical checks rather than training bots against temporary maps.
...
- Testing vocabulary: `npm run typecheck`, `npm run build`, and `npm run test:e2e` are hard pass/fail deploy gates. `npm run ai:campaign` is a heuristic playtest tool; its completion percentages are evidence for design priorities, not proof that the game is “passing.” Latest public AI JSON is `orchard-ai-gravity-priority-v1`: 6 Orchard-focused runs, 100% objective completion, 0 soft-lock risks, 0 deaths, and a +66.7 point improvement over `orchard-phased-signs-v1`. The broader rebuilt region sample `ai-objective-tuning-v2` reached 45.8% across Miller/Motel/Town/Orchard with 0 soft locks/deaths. Extended AI campaigns remain pending until route/objective clarity is stable enough for meaningful overnight evidence.
